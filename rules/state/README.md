...
- **try_enter(state: int, duration: float = 0.0, cause: int = JUMP) → bool** — Attempt to enter a state. Durationless states (IDLE, MOVING, DEAD, and CROWD_CONTROLLED) are entered normally regardless of `duration`, and never retain a `remaining` timer even if a positive `duration` is passed - they simply have no expiry. Every other state requires `duration > 0` and is rejected otherwise. Also returns false if current state is DEAD, or if `state` is invalid. Re-entering the current state returns true without emitting.
- **tick(delta: float) → void** — Advance time. Decrements remaining if tracking a duration; transitions to IDLE when duration expires. Never fires the expiry transition for a durationless state (including DEAD, which is terminal), since those never carry a positive `remaining` to begin with; the expiry path also never emits `state_changed(X, X)`.
...
- Unrecognized state names, column names, or policy values cause `push_error()` and set the detectable `load_failed` flag at load time; `can()`, `movement_policy()`, and `hard_cc_policy()` all check it and fail safe (return `false` / `"no"`) rather than silently reporting stale or partial data. Policy values are validated per column against that column's known vocabulary (see the Columns section above) - the same load-time treatment as unrecognized state names - so a bad value can never reach a use-site check unnoticed.
- Zero-or-negative duration is rejected only for states that require a duration to be meaningful (BASIC_ATTACK_WINDUP, BASIC_ATTACK_RECOVERY, ABILITY_CAST, ABILITY_CHANNEL, DASHING, AIRBORNE). IDLE, MOVING, DEAD, and CROWD_CONTROLLED may be entered with any duration value (including a positive one) and simply have no expiry: `try_enter()` forces their `remaining` to `0.0` regardless of what was passed in, so `tick()` can never fire an expiry transition for them.
- `DEAD` is terminal and reachable through `try_enter()` like any other durationless state: once entered, every `can()` query returns false, further `try_enter()` calls always return false, and `tick()` can never expire it back out (see previous point). Only `revive()` exits.
