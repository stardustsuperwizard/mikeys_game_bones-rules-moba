...
static func validate_ability(resource_path: String, ability: MobaAbility) -> Array[String]:
	...
	if ability.resource_cost < 0.0:
		violations.append("%s: resource_cost (negative: %s)" % [resource_path, ability.resource_cost])
	
	# Check magnitude-style fields are in range 0.0-1.0
	if ability.magnetism < 0.0 or ability.magnetism > 1.0:
		violations.append("%s: magnetism (out of range 0.0-1.0: %s)" % [resource_path, ability.magnetism])
	
	if ability.refund_resource_on_cancel < 0.0 or ability.refund_resource_on_cancel > 1.0:
		violations.append("%s: refund_resource_on_cancel (out of range 0.0-1.0: %s)" % [resource_path, ability.refund_resource_on_cancel])
	...
	if ability.aim_assist == MobaAbility.AimAssist.SOFT_LOCK:
		if ability.aim_cone_degrees <= 0.0 and ability.magnetism <= 0.0:
			violations.append("%s: SOFT_LOCK aim_assist missing aim_cone_degrees or magnetism" % [resource_path])
	...
	if not ability.buffs.is_empty() or not ability.debuffs.is_empty():
		...
		if ability.max_stacks < 1:
			violations.append("%s: max_stacks is invalid (< 1: %s)" % [resource_path, ability.max_stacks])
	...
	if ability.targeting_type == MobaAbility.TargetingType.CHANNELED:
		if ability.channel_duration <= 0.0:
			violations.append("%s: CHANNELED targeting_type missing/zero channel_duration" % [resource_path])
...
static func _validate_crowd_control(resource_path: String, cc: MobaCrowdControlSpec) -> Array[String]:
	var violations: Array[String] = []
	
	if cc.magnitude < 0.0 or cc.magnitude > 1.0:
		violations.append("%s: crowd_control.magnitude (out of range 0.0-1.0: %s)" % [resource_path, cc.magnitude])
	
	return violations

## Validates a stat modifier's fields.
static func _validate_stat_modifier(resource_path: String, modifier: MobaStatModifier, context: String) -> Array[String]:
	var violations: Array[String] = []
	...
	if modifier.stacking == MobaStatModifier.Stacking.STACK:
		if modifier.max_stacks < 1:
			violations.append("%s: %s max_stacks is invalid when stacking=STACK (< 1: %s)" % [resource_path, context, modifier.max_stacks])
