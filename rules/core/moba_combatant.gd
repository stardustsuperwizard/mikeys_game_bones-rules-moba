...
func _apply_hard_cc(
	...
	if state_machine != null and state_machine.current_state == MobaState.ABILITY_CAST:
		if _cast_in_progress != null and _cast_in_progress.ability.cancellable_by_hard_cc:
			cancel_cast()
...
## Cancel an in-progress cast and apply the on_cancel outcome (resource refund, cooldown change).
## A no-op if no cast is in progress. Returns without error.
...
func _refund_and_undo_cooldown(ability_id: StringName, refund_amount: float) -> void:
	restore_resource(refund_amount)
...
## Advance the in-progress cast by delta seconds and resolve it when it reaches time 0.
## Calls _apply_cast_damage_and_effects() synchronously when the cast expires.
...
func _resolve_cast() -> void:
	...
	_apply_cast_damage_and_effects(ability, resolved_target)


## Apply damage and effects from a resolved cast.
## This is the deferred resolution step for cast_time > 0 abilities.
func _apply_cast_damage_and_effects(ability: MobaAbility, target: Node) -> void:
	# Step 1: Apply damage
	_apply_cast_damage(ability, target)

	# Step 2: Apply effects (crowd control, buffs, debuffs)
	_apply_cast_effects(ability, target)


## Apply damage from a resolved cast.
func _apply_cast_damage(ability: MobaAbility, target: Node) -> void:
	...
	var target_combatant := target.get_node_or_null("MobaCombatant") as MobaCombatant
...
func _compute_scaled_damage(ability: MobaAbility) -> float:
	...
	for stat_name in ability.scaling:
		...
		amount += ratio * get_stat(StringName(stat_name))
	return amount
...
func _apply_cast_effects(ability: MobaAbility, target: Node) -> void:
	var target_combatant := target.get_node_or_null("MobaCombatant") as MobaCombatant

	# Apply crowd control from ability.crowd_control to the target
	if ability.crowd_control != null and target_combatant != null:
		target_combatant.apply_crowd_control(ability.crowd_control, self)

	# Apply buffs to the caster's combatant (self)
	var caster_effects := get_effect_container()
	...
	if ability.heal_amount > 0.0:
		apply_healing(ability.heal_amount)

	# Apply shield to the caster
	if ability.shield_amount > 0.0:
		apply_shield(ability.shield_amount, StringName(ability.id), ability.duration)
