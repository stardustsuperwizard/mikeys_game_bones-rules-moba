## Test suite for the shield pool on MobaCombatant.
##
## Covers: shield pool creation, total_shield calculation, apply_shield,
## shield consumption in the damage pipeline (shortest-remaining-duration
## first), shield expiry through tick(), and shield_changed emission.
##
## Split out of sustain_test.gd when that file passed the gdlint
## max-file-lines cap. This suite is the shield pool's own mechanics; the
## sustain economics that spend and grant it -- lifesteal, omnivamp, direct
## healing -- and the Force Barrier / Field Dressing ability-pipeline
## integration stayed in sustain_test.gd. The two fixture helpers below are
## copied from it verbatim rather than shared, matching how
## crowd_control_test.gd and crowd_control_displacement_test.gd each carry
## their own.
class_name ShieldTest
