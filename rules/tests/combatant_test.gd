...
static func _test_physical_armor_mitigation() -> Array[String]:
	...
	if not _approx_equal(combatant._current_health, expected_health, 0.01):
		violations.append("physical_armor_mitigation: expected ~%f, got %f" % [expected_health, combatant._current_health])
...
static func _test_damage_type_routing() -> Array[String]:
	...
	if not _approx_equal(actual_reduction1, expected_reduction, 0.01):
		violations.append("damage_type_routing: PHYSICAL mitigation expected %f, got %f" % [expected_reduction, actual_reduction1])
	
	if not _approx_equal(actual_reduction2, expected_reduction, 0.01):
		violations.append("damage_type_routing: MAGICAL mitigation expected %f, got %f" % [expected_reduction, actual_reduction2])
...
static func _test_true_damage_ignores_defenses() -> Array[String]:
	...
	if not is_equal_approx(combatant._current_health, expected_health):
		violations.append("true_damage_ignores_defenses: expected %f, got %f" % [expected_health, combatant._current_health])
...
static func _test_crit_before_mitigation() -> Array[String]:
	...
	if not _approx_equal(combatant._current_health, expected_health, 0.01):
		violations.append("crit_before_mitigation: expected %f, got %f" % [expected_health, combatant._current_health])
...
static func _test_seeded_rng() -> Array[String]:
	...
	if not _approx_equal(reduction1, reduction2, 0.0001):
		violations.append("seeded_rng: different results for same seed (run1=%f, run2=%f)" % [reduction1, reduction2])
