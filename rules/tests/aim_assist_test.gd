...
## Helper to create a mock Node3D-like object for testing.
class MockTarget:
	...
	func is_instance_valid(_self) -> bool:
		return true
...
static func run() -> bool:
	...
	all_violations.append_array(_test_outside_cone_ignored())

	# Test 3: Nearest-in-cone picks smallest angular deviation
	all_violations.append_array(_test_nearest_in_cone_angular_selection())

	# Test 4: Magnetism 0.0 returns raw direction
	all_violations.append_array(_test_magnetism_zero_returns_raw_direction())

	# Test 5: Magnetism 1.0 returns exact direction to target
	all_violations.append_array(_test_magnetism_one_returns_exact_direction())

	# Test 6: Magnetism 0.5 returns angular halfway via slerp
	all_violations.append_array(_test_magnetism_half_returns_angular_halfway())
...
static func _test_no_candidate_in_range() -> Array[String]:
	...
	var result := MobaAimAssist.select_nearest_in_cone(
		raw_aim, caster_pos, candidates, cone_angle
	)

	if result != null:
		violations.append(
			"No candidates: expected null, got %s" % result
		)
...
## Test 2: Candidate outside cone ignored even if metrically closest.
static func _test_outside_cone_ignored() -> Array[String]:
	var violations: Array[String] = []
	...
	# Create a mock candidate outside the cone
	var outside_target := Node3D.new()
	outside_target.global_position = Vector3(1.0, 1.0, 0.0)  # ~45 degrees, outside 30-degree cone
	...
	if result != null:
		violations.append(
			"Outside cone: expected null, got %s" % result
		)
...
static func _test_nearest_in_cone_angular_selection() -> Array[String]:
	var violations: Array[String] = []
	...
	var close_target := Node3D.new()
	close_target.global_position = Vector3(1.0, 0.5, 0.0)  # Close, but ~26.5 degrees off

	# Candidate 2: Far but more aligned (smaller angle)
	var far_target := Node3D.new()
	far_target.global_position = Vector3(2.0, 0.1, 0.0)  # Far, but ~2.9 degrees off
	...
	if result != far_target:
		violations.append(
			"Nearest-in-cone: expected far_target (better angle), got %s" % result
		)
...
static func _test_magnetism_zero_returns_raw_direction() -> Array[String]:
	var violations: Array[String] = []
	...
	var result := MobaAimAssist.resolve_direction(
		raw_aim, target, caster_pos, magnetism
	)

	if not _approx_equal(result.x, raw_aim.x) or not _approx_equal(result.y, raw_aim.y) or not _approx_equal(result.z, raw_aim.z):
		violations.append(
			"Magnetism 0.0: expected raw direction %v, got %v" % [raw_aim, result]
		)
...
## Test 5: Magnetism 1.0 returns exact direction to target.
static func _test_magnetism_one_returns_exact_direction() -> Array[String]:
	var violations: Array[String] = []
	...
	if not _approx_equal(result.x, to_target.x) or not _approx_equal(result.y, to_target.y) or not _approx_equal(result.z, to_target.z):
		violations.append(
...
static func _test_magnetism_half_returns_angular_halfway() -> Array[String]:
	var violations: Array[String] = []
...
static func _test_device_multiplier_loading() -> Array[String]:
	...
	if not _approx_equal(mouse_mult, 0.23, 0.001):
		violations.append(
			"Device multiplier: mouse expected 0.23, got %f" % mouse_mult
		)

	if not _approx_equal(gamepad_mult, 0.67, 0.001):
		violations.append(
			"Device multiplier: gamepad expected 0.67, got %f" % gamepad_mult
		)

	if not _approx_equal(touch_mult, 1.0, 0.001):
		violations.append(
			"Device multiplier: touch expected 1.0, got %f" % touch_mult
		)
...
static func _test_effective_magnetism_clamp() -> Array[String]:
	...
	if not _approx_equal(result1, 1.0):
		violations.append(
			"Clamp test 1: expected 1.0, got %f" % result1
		)
	...
	if not _approx_equal(result2, 1.0):
		violations.append(
			"Clamp test 2: expected 1.0, got %f" % result2
		)

	# Test case 3: ability_magnetism=0.5, device_multiplier=1.5 → should clamp to 1.0
	var result3 := MobaAimAssist.effective_magnetism(0.5, 1.5)
	if not _approx_equal(result3, 1.0):
		violations.append(
			"Clamp test 3: expected 1.0, got %f" % result3
		)
	...
	if not _approx_equal(result4, 0.5):
		violations.append(
			"Clamp test 4: expected 0.5, got %f" % result4
		)
...
static func _test_free_aim_unaffected_by_device_multiplier() -> Array[String]:
	...
	for device in devices:
		...
		if not _approx_equal(result, 0.0):
			violations.append(
				"Free aim: %s multiplier %.2f should yield 0.0, got %f" % [device, multiplier, result]
			)
...
static func _test_all_device_multipliers_within_bounds() -> Array[String]:
	...
	for device in devices:
		...
		if multiplier > 1.0:
			violations.append(
				"Bounds check: %s multiplier %.2f exceeds 1.0" % [device, multiplier]
			)
...
static func _test_locked_target_resolution() -> Array[String]:
	...
	if not _approx_equal(result1.x, locked_direction.x) or not _approx_equal(result1.y, locked_direction.y) or not _approx_equal(result1.z, locked_direction.z):
		violations.append(
	...
	if not _approx_equal(result2.x, raw_aim.x) or not _approx_equal(result2.y, raw_aim.y) or not _approx_equal(result2.z, raw_aim.z):
		violations.append(
