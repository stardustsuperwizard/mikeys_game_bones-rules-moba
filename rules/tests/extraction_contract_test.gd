...
static func run() -> bool:
	...
	for file_path in files:
		...
		if not (file_path.ends_with(".gd") or file_path.ends_with(".json") or file_path.ends_with(".tres")):
			continue
	...
	printerr("Files in rules/ must not reference res://scripts/, res://scenes/, or res://resources/")
	printerr("")
