...
static func _test_resource_seeding() -> Array[String]:
	...
	if not is_equal_approx(actual_resource, expected_resource):
		violations.append(
			(
				"resource_seeding: current_resource mismatch (expected %f, got %f)"
	...
	if not is_equal_approx(actual_max, expected_max):
		violations.append(
			(
				"resource_seeding: maximum_resource mismatch (expected %f, got %f)"
...
static func _test_spend_resource() -> Array[String]:
	...
	if not is_equal_approx(actual_resource, expected_resource):
		violations.append(
			"spend_resource: expected %f, got %f" % [expected_resource, actual_resource]
		)
...
static func _test_spend_resource_insufficient() -> Array[String]:
	...
	if not is_equal_approx(combatant.current_resource, 20.0):
		violations.append(
			(
				(
					"spend_resource_insufficient: resource should not change when"
					+ " spend fails (expected 20.0, got %f)"
				)
				% combatant.current_resource
			)
		)
...
static func _test_restore_resource_clamped() -> Array[String]:
	...
	if not is_equal_approx(combatant.current_resource, max_resource):
		violations.append(
			(
				"restore_resource_clamped: expected %f, got %f"
...
static func _test_regeneration_accumulation() -> Array[String]:
	...
	if not is_equal_approx(combatant.current_resource, max_resource):
		violations.append(
			(
				"regeneration_accumulation: after 10 seconds of 3.0 regen, expected %f, got %f"
				% [max_resource, combatant.current_resource]
			)
		)
...
static func _test_partial_tick_accumulation() -> Array[String]:
	...
	combatant1._current_resource = combatant1._runtime_stat_block.get_stat_value(
		MobaStatBlock.RESOURCE
	)

	# Spend first so regeneration has room to accumulate below the clamp --
	# starting at maximum, both paths would just clamp to the same ceiling
	# and this comparison would pass no matter how tick() accumulates.
	combatant1.spend_resource(30.0)
	combatant1.tick(1.0)
	...
	combatant2._current_resource = combatant2._runtime_stat_block.get_stat_value(
		MobaStatBlock.RESOURCE
	)

	combatant2.spend_resource(30.0)
	for _i in range(10):
		combatant2.tick(0.1)
	var result2 = combatant2.current_resource

	# Both paths regenerate 3.0/sec over 1 total second (+3.0, well under
	# the 250 maximum after spending 30), so this actually exercises
	# accumulation instead of both sides clamping to the same ceiling.
	# Results should be equal within float tolerance
	if not _approx_equal(result1, result2, 0.0001):
		violations.append(
			(
				"partial_tick_accumulation: single 1.0s tick gave %f, ten 0.1s ticks gave %f"
				% [result1, result2]
			)
		)
...
static func _test_health_regeneration() -> Array[String]:
	...
	var health_regen = combatant._runtime_stat_block.get_stat_value(MobaStatBlock.HEALTH_REGEN)
	var max_health = combatant._runtime_stat_block.get_stat_value(MobaStatBlock.HEALTH)

	# Tick for 10 seconds at health_regen/sec
	for _i in range(10):
		combatant.tick(1.0)

	var health_after_regen = combatant._current_health
	var expected_health = minf(health_after_damage + health_regen * 10.0, max_health)

	if not _approx_equal(health_after_regen, expected_health, 0.01):
		violations.append(
			(
				"health_regeneration: expected %f after 10s at %f/sec (before %f), got %f"
				% [expected_health, health_regen, health_after_damage, health_after_regen]
			)
		)

	# Separately prove clamping: ticking far past full must not exceed maximum.
	...
	if not is_equal_approx(combatant._current_health, max_health):
		violations.append(
			(
				(
					"health_regeneration: expected health clamped at maximum %f after"
					+ " extended ticking, got %f"
				)
				% [max_health, combatant._current_health]
			)
		)
...
static func _test_dead_combatant_no_health_regen() -> Array[String]:
	...
	if combatant._current_health > health_when_dead:
		violations.append(
			(
				(
					"dead_combatant_no_health_regen: dead combatant should not"
					+ " regenerate health (before %f, after %f)"
				)
				% [health_when_dead, combatant._current_health]
			)
		)
